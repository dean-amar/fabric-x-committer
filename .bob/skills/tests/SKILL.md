...
- **ACL (this repo):** `auth.NewServiceTestEnv` (`service/auth/test_exports.go`) runs a real AuthService — use `MintToken` for a token and `ACLClient` for the resource server's `auth:` section, then `acl.ContextWithToken` to authorize one RPC. Don't stub the AuthService.

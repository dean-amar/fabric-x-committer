...
// NewACLUpdater creates an ACLUpdater. requiresACL fixes, for the updater's lifetime, whether the
// owning service enforces ACL: services that need enforcement (sidecar, query) pass true; services
// that only need the dynamic TLS client-CA refresh (orderer) pass false. Enforcement therefore
// travels with the updater, decided at construction, rather than being set at registration time.
func NewACLUpdater(requiresACL bool) *ACLUpdater {
	return &ACLUpdater{requiresACL: requiresACL}
}
...
func RegisterACLUpdater(d *ACLProvider, updater *ACLUpdater) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.updater = updater
}

...
func (vc *ValidatorCommitterService) Run(ctx context.Context) error {
	...
	g.Go(func() error {
		...
		if vc.config.Monitoring.Retry == nil {
			return vc.metrics.StartPrometheusServer(eCtx, vc.config.Monitoring.Server, vc.monitorQueues)
		}
		return vc.config.Monitoring.Retry.Execute(eCtx, func() error {
			return vc.metrics.StartPrometheusServer(eCtx, vc.config.Monitoring.Server, vc.monitorQueues)
		})
	})

...
An optional eighth service, the **Auth Service**, provides access control: it authenticates clients from a signed envelope, issues certificate-bound tokens, and answers per-RPC authorization decisions for the API-exposing services (Query Service and Sidecar). It is enabled per service via an `auth:` configuration section; when omitted, those services serve without ACL checks. See [Auth Service](auth-service.md).

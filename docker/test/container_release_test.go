	...
	for _, mode := range []string{connection.MutualTLSMode} {
		...
		t.Run(fmt.Sprintf("tls-mode:%s", mode), func(t *testing.T) {
			...
			for _, node := range []string{
				...
				// stop and remove the container if its already exists.
				stopAndRemoveContainersByName(ctx, t, createDockerClient(t), assembleContainerName(node, mode))

				switch node {
				case "db":
					conn := dbtest.CreateAndStartSecuredDatabaseNode(ctx, t, dbtest.PostgresDBType, networkName, "db", credsFactory)
...
func startCommitterNodeWithReleaseImage(
	...
	t.Helper()
	_, serverCredsPath := params.credsFactory.CreateServerCredentials(t, params.tlsMode, params.node, testutils.CertStyleDefault)

	...
	tests := []struct {
		...
		expectedConfig: &sidecar.Config{
			...
			Monitoring: &monitoring.Config{
				Server: newServerConfig("localhost", 2114),
			},
		...
		expectedConfig: &sidecar.Config{
			...
			Monitoring: &monitoring.Config{Server: newServerConfigWithDefaultTLS("sidecar", 2114)},
			Orderer: ordererconn.Config{
				ChannelID: "mychannel",
...
func TestReadConfigCoordinator(t *testing.T) {
	...
	tests := []struct {
		...
		expectedConfig: &coordinator.Config{
			...
			Monitoring: &monitoring.Config{Server: newServerConfig("localhost", 2119)},
		...
		expectedConfig: &coordinator.Config{
			...
			Monitoring:         &monitoring.Config{Server: newServerConfigWithDefaultTLS("coordinator", 2119)},
...
func TestReadConfigVC(t *testing.T) {
	...
	tests := []struct {
		...
		expectedConfig: &vc.Config{
			...
			Monitoring: &monitoring.Config{Server: newServerConfig("localhost", 2116)},
		...
		expectedConfig: &vc.Config{
			...
			Monitoring: &monitoring.Config{Server: newServerConfigWithDefaultTLS("vc", 2116)},
...
func TestReadConfigVerifier(t *testing.T) {
	...
	tests := []struct {
		...
		expectedConfig: &verifier.Config{
			...
			Monitoring: &monitoring.Config{Server: newServerConfig("localhost", 2115)},
		...
		expectedConfig: &verifier.Config{
			...
			Monitoring: &monitoring.Config{Server: newServerConfigWithDefaultTLS("verifier", 2115)},
			ParallelExecutor: verifier.ExecutorConfig{
				BatchSizeCutoff:   50,
...
func TestReadConfigQuery(t *testing.T) {
	...
	tests := []struct {
		...
		expectedConfig: &query.Config{
			...
			Monitoring:            &monitoring.Config{Server: newServerConfig("localhost", 2117)},
			Database:              defaultDBConfig(),
			MinBatchKeys:          1024,
		...
		expectedConfig: &query.Config{
			...
			Monitoring:            &monitoring.Config{Server: newServerConfigWithDefaultTLS("query", 2117)},
			Database:              defaultSampleDBConfig(),
			MinBatchKeys:          1024,
			MaxBatchWait:          100 * time.Millisecond,
			ViewAggregationWindow: 100 * time.Millisecond,

...
// RegisterService registers for the verifier'OrdererBundle GRPC services.
...
func (m *Verifier) StartStream(stream servicepb.Verifier_StartStreamServer) error {
	...
	logger.Infof("Starting verifier stream on server %OrdererBundle for client %OrdererBundle",
...
// ServerEndpoint returns the stream'OrdererBundle server endpoint.
